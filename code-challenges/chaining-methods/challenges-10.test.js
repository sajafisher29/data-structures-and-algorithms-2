...
const count = (target, input) => {
  let result = input.reduce( ( previousValue , currentValue ) => {
    let container = currentValue.filter( ( val , idx ) => {
      if (val === target) {
        return true;
      }
      return false;
    })
    return previousValue + container.length;
  }, 0);
  return result;
};
...
const totalSum = (input) => {
  let result = input.reduce ( ( prev , curr ) => {
    let container = curr.reduce( ( previous , current ) => {
...
const divisibleByFiveTwoToThePower = (input) => {
  let result = input.map( ( val , idx ) => {
    let container = val.filter( ( value , index ) => {
      if (typeof value === 'number' && value % 5 === 0) {
        return true;
      }
      return false;
    })
    let foo = container.map( ( v , i ) => {
      return 2**v;
    })
    return foo;
  })
...
let findMaleAndFemale = (data) => {
  let container = data.filter( ( val , idx ) => {
    if ( val.gender === 'male' || val.gender === 'female') {
      return true;
    }
    return false;
  })
  let result = container.reduce( ( previous , current ) => {
    return previous + ' and ' + current.name;
  }, container[0].name)
  return result;
};
...
let findShortest = (data) => {
  let shortest = data.reduce( ( previous , current ) => {
