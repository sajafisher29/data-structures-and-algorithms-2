...
const validateEmail = (email) => {
  let validator = /^\w+\.\w+\@|^\w+\@/g
  let validator2 = /.com$|.net$|.org$/
  if (validator.test(email) && validator2.test(email)) {
    return true;
  }
  return false;
};
...
const validatePhoneNumber = (phoneNumber) => {
  let formatAreaCode = /^\([0-9]{3}\)|^[0-9]{3}-|^[0-9]{3}\s|^[0-9]{6}-/g
  let formatSeven= /\s[0-9]{3}\s[0-9]{4}$|-[0-9]{3}-|\s[0-9]{3}-[0-9]{4}$|[0-9]{7}|[0-9]\s[0-9]{4}$|[0-9]{7}/g
  let checkAll = /^[0-9]{10}$/g
  if (formatAreaCode.test(phoneNumber) && formatSeven.test(phoneNumber) || checkAll.test(phoneNumber)) {
    return true;
  }
  return false;
};
