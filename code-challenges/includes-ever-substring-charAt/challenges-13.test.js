...
const allHappy = (arr) => {
  let results = arr.map( val => val.includes(':)'))
...
const findAnything = (arr, target) => {
  let results = arr.filter( val => val.includes(target))
  return results;
};
...
const findEvery = (arr, target) => {
  let results = arr.map( val => val.includes(target));
  return !results.includes(false);
};
...
const unenrollBrook = (arr) => {
  let resultArr = arr.map( val => {
    return val.filter( str => !str.includes('Brook'))
  })
  return resultArr;
};
