...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i < recipe.ingredients.length; i++) {
    let ingred = recipe.ingredients[i];
    let index = ingred.indexOf(' ', 4 );
    console.log(ingred, index);
    result.push(ingred.slice(index - ingred.length + 1));
  }
  return result;
};
...
const splitFoods = (recipe) => {
  let result = listFoods(recipe); // You may also use other string or array methods.
  return result;
};
